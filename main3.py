...
def clean_page1_keep_only_title(doc: Document, title: str) -> bool:
    ...
    Title is centered horizontally and positioned at the top of page 1.
    ...
    try:
        ...
        # CRITICAL: Get the document body element and ensure it's clean
        # We need to access the document body directly to ensure title is first
        try:
            body = doc.element.body
            # Remove all paragraph elements from body except we'll add title back
            # Get all paragraph elements
            paras = body.xpath('.//w:p')
            # Remove all existing paragraphs from body to ensure clean start
            for para_elem in paras:
                try:
                    ...
                    pass
            print(f"   🧹 Cleared all existing paragraphs from document body")
        except Exception as e:
            print(f"   ⚠️  Could not clear document body directly: {e}")
            # Fallback: remove paragraphs the standard way
        ...
        # Configure paragraph formatting for title at top of page 1
        title_para.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.CENTER  # Center horizontally
        title_para.paragraph_format.space_before = Pt(0)  # No space before - title at top of page
        ...
        title_para.paragraph_format.page_break_before = False  # Ensure no page break before
        title_para.paragraph_format.widow_control = False  # Disable widow control
        
        # CRITICAL: Ensure no page break before or in this paragraph
        try:
            ...
            for run in title_para.runs:
                try:
                    ...
                    pass
            # Also check the paragraph element itself for page breaks
            para_element = title_para._element
            br_elements = para_element.xpath('.//w:br[@w:type="page"]')
            for br in br_elements:
                try:
                    br.getparent().remove(br)
                except:
        ...
        if first_para_text == title:
            ...
            print(f"   ✅ Title will appear on PAGE 1 (centered horizontally, at top)")
        else:
            ...
            try:
                ...
                title_para.paragraph_format.space_before = Pt(0)  # Title at top of page
        ...
        print(f"   ✅ Added energetic title to page 1 (centered horizontally, at top, ONLY title): '{title}'")
        ...
        print(f"   ✅ Title will appear on PAGE 1, centered horizontally at the top of the page")
        return True
    except Exception as e:
...
def clean_page2_remove_template_content(doc: Document, after_title_idx: int = 1):
    ...
    IMPORTANT: This function will NEVER remove the title at index 0.
    """
    print(f"\n🧹 Cleaning page 2 - removing ALL template content...")
    
    # CRITICAL: Protect the title at index 0 - never remove it
    # The title should always be at index 0 on page 1
    protected_title_idx = 0
    ...
    # Start from index 1 to protect the title at index 0
    start_idx = max(1, after_title_idx + 1)  # Always start from at least index 1
    ...
    for idx in range(start_idx, end_idx):
        ...
        # CRITICAL: Never remove the title at index 0
        if idx == protected_title_idx:
            print(f"   🔒 Protecting title at index {idx} from removal")
            continue
        ...
        # BUT NEVER remove index 0 (the title)
        if text or len(para.runs) > 0:
            ...
            # Also remove if it looks like template content
                ...
                # But only if it's not the protected title index
                elif idx < start_idx + 80 and idx != protected_title_idx:  # Remove first 80 paragraphs after title
                    paragraphs_to_remove.append(idx)
    
    # Remove paragraphs in reverse order
    removed_count = 0
    for idx in reversed(paragraphs_to_remove):
        # CRITICAL: Double-check - never remove the title at index 0
        if idx == protected_title_idx:
            print(f"   🔒 SKIPPING: Protected title at index {idx} from removal")
            continue
        
        try:
            if idx < len(doc.paragraphs):
                # Additional safety check: verify we're not removing the title
                para_text = doc.paragraphs[idx].text.strip()
                # If this looks like it might be a title (centered, large text, etc.), be cautious
                # But proceed if it's clearly template content
                p_element = doc.paragraphs[idx]._element
                parent = p_element.getparent()
                if parent is not None:
                    parent.remove(p_element)
    ...
    # Final verification: ensure title is still at index 0
    if len(doc.paragraphs) > 0:
        first_para_text = doc.paragraphs[0].text.strip()
        print(f"   🔍 Verification: First paragraph after cleaning: '{first_para_text[:60]}...'")
        if len(first_para_text) > 0:
            print(f"   ✅ Title protection: First paragraph preserved (should be the title)")
    ...
    print(f"   ✅ Title at index 0 is protected and preserved")
